...
class RaycastVehicle {
  ...
  castRay(wheel) {
    ...
    const chassisBody = this.chassisBody;
    let depth = -1;
    // 探测距离覆盖完整悬架行程：rest + travel + radius。
    // 否则车身被路面（Trimesh/交叉口）支撑抬高时，轮子 raycast 够不到地面，
    // 会悬空无接触、驱动空转（发夹弯入口“开不动”的根因之一）。
    ...
    // 道路兜底：raycast miss 时按最近道路高度补一个接触，
    // 避免交叉口/网格误差导致轮子悬空（车身会被悬挂拉回路面）
    if (!raycastResult.body && this.world.roadFallback) {
      this.world.roadFallback(wheel, raycastResult);
    }
    ...
    if (object) {
      // 平滑法线：用游戏地形的梯度替代三角片的平坦法线，消除坡面震动
      if (raycastResult.hitNormalWorld.y > 0.5 && this.world.contactNormal) {
        this.world.contactNormal(
